...
public class ContainerBag extends Container {
  ...
  public ContainerBag(EntityPlayer player, IInventory inv, int meta) {
    ...
    for (int i = 0; i < 6; i++) {
      for (int j = 0; j < 9; j++) {
        this.addSlot(new SlotBag(inv, j + i * 9, 8 + j * 18, 14 + i * 18));
      }
    }
  ...
  public boolean canInteractWith(@Nonnull EntityPlayer entityplayer) {
    ...
    return !item.isEmpty() && item.getItem() instanceof ItemHDBag
        && ((ItemHDBag) item.getItem()).getDyeColor().getId() == metaDmg;
  }
  ...
  public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int par2) {
    ...
    if (slot != null && slot.getHasStack()) {
      ...
      if (par2 < this.BagInv.getSizeInventory()) {
        if (!this
            .mergeItemStack(itemstack1, this.BagInv.getSizeInventory(), this.inventorySlots.size(),
                true)) {
