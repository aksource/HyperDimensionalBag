...
public class Recipes {
    ...
    public void registerRecipes(RegistryEvent.Register<IRecipe> event) {
        ...
        for (int i = 0; i < 15; i++) {
            int[] oreIDs = OreDictionary.getOreIDs(new ItemStack(Items.DYE, 1, i));
            for (int oreId : oreIDs) {
                String oreName = OreDictionary.getOreName(oreId);
                if (oreName.matches("^dye.+")) {
                    registry.register(new ShapelessOreRecipe(hdBagRl,
                            ...
                            oreName).setRegistryName(new ResourceLocation(hdBagRl.getResourcePath() + i)));
                }
            }
        }
        if (!hardRecipe) {
