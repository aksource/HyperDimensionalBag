        ...
        for (int i = 0; i < inv.getSizeInventory(); i++) {
            ...
            if (checkValidBlock(targetBlockStack, item, drops)) {
                ...
                return true;
                    ...
                    && ItemStorageBox.peekItemStackAll(item).stackSize > 1) {
    ...
    public EnumActionResult onItemUse(ItemStack itemStack, EntityPlayer playerIn, World worldIn, BlockPos blockPos, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (targetBlock == null
            ...
            if (!worldIn.isRemote) {
                ...
                playerIn.addChatMessage(new TextComponentString(registerBlock));
            }
        ...
        if (EnumBuildMode.getMode(mode) == EnumBuildMode.exchange) {
            ...
            searchExchangeableBlock(worldIn, playerIn, targetBlockStack, blockPos, blockPos, side, side, itemStack);
            getDroppedBlock(worldIn, playerIn);
        }
        ...
        if (EnumBuildMode.getMode(mode) == EnumBuildMode.wall) {
            blockPosList = getNextWallBlockPosList(worldIn, playerIn, blockPos, side, range, allMode);
            putBlockToBlockPosList(worldIn, playerIn, blockPosList, itemStack, targetBlockStack, allMode);
        }
        ...
        if (EnumBuildMode.getMode(mode) == EnumBuildMode.cube) {
            blockPosList = getNextCubeBlockPosList(worldIn, playerIn, blockPos, side, range, allMode);
            putBlockToBlockPosList(worldIn, playerIn, blockPosList, itemStack, targetBlockStack, allMode);
        }
        return EnumActionResult.SUCCESS;
    }
